...
const parseOrderDate = (date) => {
  ...
  // Sun, 25 Jul
  return (
    parsedDate.toDateString().slice(0, 3) +
    ", " +
    parsedDate.getDate() +
    " " +
    parsedDate.toDateString().slice(4, 7)
  );
};
...
const DateWiseOrders = (props) => {
  ...
  if (isReplacement) {
    Object.keys(groupedOrders).forEach((date) => {
      const status = groupedOrders[date].some(
        (order) => order.status === "Replacement Successful"
      )
        ? "Replacement Successful"
        : groupedOrders[date].some((order) => order.status === "Picked")
        ? "Picked"
                ...
                >
                  <div className={Styles.orderSummary}>
                    ...
                    <p className={Styles.totalProducts}>
                      Total products: {groupedOrders[date].length}
                    </p>
                    <p
                      className={
                        Styles.status +
                        " " +
                        Styles[
                          `status${groupedOrders[date].status.replace(" ", "")}`
                        ]
                      }>
                      Status: {groupedOrders[date].status}
                    </p>
                  </div>
                ...
                {expandedDate === date && (
                  <div className={Styles.orderDetails}>
                    ...
                    <button
                      ...
                      onClick={() =>
                        router.push(
                          `/order/${deliveryStatus}/${getDateString(
                            groupedOrders[date][0].date
                          )}`
                        )
                      }>
